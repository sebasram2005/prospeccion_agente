"""
Supabase client for Vertical 3 — HMLV Manufacturers.
"""
...
class LeadsRepository:
    ...
    async def is_duplicate(self, url: str) -> bool:
        try:
            ...
            logger.error("dedup_check_failed", url=url, error=str(exc), vertical="hmlv")
            return False
    ...
    async def insert_raw_lead(self, data: dict) -> str | None:
        try:
            ...
            logger.error("insert_raw_lead_failed", error=str(exc), vertical="hmlv")
            return None
    ...
    async def mark_as_processed(self, lead_id: str) -> None:
        try:
            ...
            logger.error(
                "mark_processed_failed", lead_id=lead_id, error=str(exc), vertical="hmlv"
            )
    ...
    async def insert_qualified_lead(self, data: dict) -> str | None:
        try:
            ...
            logger.error("insert_qualified_lead_failed", error=str(exc), vertical="hmlv")
            return None
    ...
    async def create_email_queue_entry(self, data: dict) -> str | None:
        try:
            ...
            logger.info("email_queued", queue_id=queue_id, vertical="hmlv")
            return queue_id
        except Exception as exc:
            logger.error("create_email_queue_failed", error=str(exc), vertical="hmlv")
            return None
    ...
    async def fetch_unqualified_leads(self) -> list[dict]:
        ...
        try:
            ...
            logger.info("unqualified_leads_found", count=len(unqualified), vertical="hmlv")
            return unqualified
        except Exception as exc:
            logger.error("fetch_unqualified_failed", error=str(exc), vertical="hmlv")
            return []
    ...
    async def is_already_emailed(self, email: str) -> bool:
        try:
            ...
            logger.error(
                "email_dedup_check_failed", email=email, error=str(exc), vertical="hmlv"
            )
            return False
    ...
    async def update_keyword_performance(self) -> None:
        ...
        try:
            ...
            if not raw.data:
                logger.info("keyword_perf_no_data", vertical="hmlv")
                return
            ...
            logger.info("keyword_perf_updated", keywords_tracked=len(stats), vertical="hmlv")

        except Exception as exc:
            logger.error("keyword_perf_update_failed", error=str(exc), vertical="hmlv")
